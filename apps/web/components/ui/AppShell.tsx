...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className="flex h-screen overflow-hidden bg-surface">
      {/* ── Sidebar ──────────────────────────────────────────────────── */}
      <aside className="w-56 flex-shrink-0 border-r border-surface-300 bg-surface-100 flex flex-col">
        {/* Logo */}
          ...
          {NAV.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon
                  size={15}
                  className={active ? 'text-brand-green-400' : 'text-zinc-500 group-hover:text-white'}
                />
                {label}
              </Link>
            );
